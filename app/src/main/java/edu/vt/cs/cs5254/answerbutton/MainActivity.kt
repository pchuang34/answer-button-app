...
//        }
        answerButtonList.zip(answerList) { btn, ans -> btn.setText(ans.textResId) }
...
//            }
//        }
        answerButtonList.zip(answerList) { btn, ans ->
            btn.setOnClickListener {
                processAnswerButtonClick(
                    ans
                )
            }
        }
...
//        }
        answerList.forEach { ans -> ans.isSelected = false }
...
//            }
//        }
        answerList.filter { ans -> !ans.isCorrect }
            .take(2)
            .forEach {
                it.isSelected = false
                it.isEnabled = false
...
//            }
//        }
        answerList.zip(answerButtonList) { ans, btn ->
            btn.isEnabled = ans.isEnabled
            btn.isSelected = ans.isSelected
            if (ans.isSelected) {
                setButtonColor(btn, SELECTED_BUTTON_COLOR)
            } else {
                setButtonColor(btn, DEFAULT_BUTTON_COLOR)
            }
            if (!ans.isEnabled) {
                btn.alpha = .5f
